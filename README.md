...
- **고아 pending** — 컨슈머 이름이 hostname 기반이라, Pod 교체 시 이전 컨슈머가 ACK하지 못한 엔트리를 자동으로 회수하지 않습니다. `XAUTOCLAIM` 도입이 **현재 인지하는 가장 큰 미구현 항목**입니다.
...
- **테스트 미비** — 분리 과정에서 테스트가 이관되지 않아 현재 테스트 코드가 없습니다.
